...
// Configuration
const START_YEAR = parseInt(process.argv[2]) || 1970;
const END_YEAR = parseInt(process.argv[3]) || 2025;
const BASE_QUERY = '(cannabis[Title/Abstract] OR cannabidiol[Title/Abstract] OR CBD[Title/Abstract] OR THC[Title/Abstract] OR marijuana[Title/Abstract] OR cannabinoid[Title/Abstract])';
const MAX_RESULTS_PER_YEAR = 10000;
const API_KEY = null; // Set if you have one for higher rate limits

console.log('🔬 PubMed Historical Backfill');
console.log('=============================');
...
for (let year = START_YEAR; year <= END_YEAR; year++) {
  ...
  const yearQuery = `${BASE_QUERY} AND (${year}[PDAT])`;
  
  // Create ingest run
  const { data: ingestRun } = await supabase
    .from('ingest_runs')
    .insert({
  ...
  console.log(`📝 Ingest Run: ${ingestRun.id}`);
  ...
  try {
    ...
    console.log(`\n1️⃣ Searching PubMed for ${year}...`);
    const searchParams = new URLSearchParams({
      db: 'pubmed',
      term: yearQuery,
      retmax: MAX_RESULTS_PER_YEAR.toString(),
      retmode: 'json',
      sort: 'pub_date',
    ...
    for (let i = 0; i < pmids.length; i += batchSize) {
      ...
      if (fetchResponse.ok) {
        ...
        if ((i + batchSize) % 400 === 0 || i + batchSize >= pmids.length) {
          console.log(`   Fetched ${Math.min(i + batchSize, pmids.length)}/${pmids.length} articles`);
        }
      }

      // Rate limiting
      if (i + batchSize < pmids.length) {
        await new Promise(resolve => setTimeout(resolve, API_KEY ? 100 : 340));
      }
    }
    ...
    for (const paper of papers) {
      ...
      try {
        const { doi, pmid, title, abstract, authors, journal, publishedDate, isOpenAccess } = paper;

        if (!title || !pmid) {
          yearErrors.push({ doi: doi || pmid || 'unknown', error: 'Missing required fields' });
          continue;
        }
        ...
        if (signalError || !signalItem) {
          yearErrors.push({ doi: doi || pmid, error: signalError?.message || 'Failed to create signal_item' });
          continue;
        }
        ...
        if (yearCreated % 50 === 0) {
          console.log(`   ✅ Created ${yearCreated} papers...`);
        }
    ...
    await supabase.from('ingest_runs').update({
      status: 'completed',
      completed_at: new Date().toISOString(),
      items_processed: yearProcessed,
      items_created: yearCreated,
      items_duplicate: yearDuplicates,
      items_error: yearErrors.length,
    }).eq('id', ingestRun.id);

    console.log(`\n✅ Year ${year} Complete:`);
    console.log(`   Processed: ${yearProcessed}`);
    console.log(`   Created: ${yearCreated}`);
    console.log(`   Duplicates: ${yearDuplicates}`);
    console.log(`   Errors: ${yearErrors.length}`);

    totalProcessed += yearProcessed;
    totalCreated += yearCreated;
    totalDuplicates += yearDuplicates;
    totalErrors += yearErrors.length;

  } catch (error) {
    console.error(`\n❌ Year ${year} failed:`, error.message);
...
console.log(`\n✨ All historical cannabis research articles have been ingested!\n`);

// Parse PubMed XML (same as before)
